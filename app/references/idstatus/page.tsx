...
const IDStatus = () => {
  ...
  const fetchData = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:5001/pin?pin=${pin}`);
          ...
          <div className="forms-container">
            <div className="signin-signup">
              <form className="sign-in-form" onSubmit={fetchData}>
                <h2 className="title">ID Card Status</h2>
                {pinError && (
        <div className="error-message">
          ...
          <p>{pinError}</p></div> )}
                ...
                <div className="input-field">
                  ...
                  <input type="text" placeholder="ID Status" name="IDStatus" id="passportStatus" value={Descr_value || ''} readOnly style={{ width: '210px' }} />
                </div>
              </form>
            </div>
          </div>
          <div className="panels-container">
            <div className="panel left-panel">
              <div className="content" data-aos="fade-up">
                <h1>Check your <br />Identity Card Status</h1>
                <p>Check the current status of your Eswatini National Identity Card to ensure it is up to date.</p>
              </div>
            </div>
          </div>
        </div>
